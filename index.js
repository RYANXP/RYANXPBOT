	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '❌Erro Ao processar a imagem!  ❌',
					Iv: '❌ Link inválido❌'
				},
				only: {
			...
			switch(command) {
				...
				case 'sticker':
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'Coloque o codigo do idioma', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Coloque um texto', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'hilih':
					...
					break
				case 'ytmp3':
					if (args.length < 1) return reply('Insira uma url do YouTube!')
				...
				case 'tagall':
					...
					break
				case 'clearall':
					if (!isOwner) return reply('Quem e Você?')
				...
				case 'add':
					...
					if (args.length < 1) return reply('Você quer adicionar uma pessoa?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar A pessoa, talvez porque o numero é privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
					...
					if (mentioned.length > 1) {
						teks = 'Pedido recebido, Irei Remover essa pessoa :\n'
				...
				case 'listadmins':
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ um adesivo de resposta ❌')
					reply(mess.wait)
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Basta tirar uma foto, mano')
					}
					break
				default:
